...
class CrawlerANS {
    ...
    static void executar() {
        println "🔍 Executando Crawler ANS..."
        ...
        def http = configure { request.uri = linkFile }
        http.get {
            response.success { _, inputStream ->
                destino.parentFile.mkdirs()
                destino.withOutputStream { out -> out << inputStream }
            }
        }
        ...
        def linkErro = doc4.select("a").find { it.text().toLowerCase().contains("tabela de erros") }
